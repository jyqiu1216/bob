...
PROCESS_CMD:
    ...
    //TSE_LOG_INFO(pstSession->m_poServLog, ("process_by_cmd: step[%u] end process: ret[%d],bNeedResponse[%d] [seq=%u]",
    //    pstSession->m_udwProcessSeq, dwRetCode, bNeedResponse, pstSession->m_stUserInfo.m_udwBSeqNo));
    ...
    if(dwRetCode != 0)
    {
        TSE_LOG_ERROR(pstSession->m_poServLog, ("process_by_cmd: reqcmd=%u, nowcom=%u, curret=%d, comret=%d [seq=%u]", \
            ...
            pstSession->m_stCommonResInfo.m_dwRetCode, pstSession->m_stUserInfo.m_udwBSeqNo));
