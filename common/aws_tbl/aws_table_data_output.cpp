...
TableDesc TbData_output::oTableDesc;

int TbData_output::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbData_output::NewObject()
{
	return new TbData_output;
}

string TbData_output::GetTableName()
{
	ostringstream oss;
...
TINT32 TbData_output::GetTableIdx()
{
	 return 0;
}

AwsMap* TbData_output::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbData_output::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbData_output::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbData_output::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbDATA_OUTPUT_FIELD_UID)
			{
...
int TbData_output::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbData_output::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbData_output::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbData_output::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbDATA_OUTPUT_FIELD_UTYPE == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/utype/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/utype/Value/N", m_nUtype);
				pUpdateItem->AddValue("/AttributeUpdates/utype/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbDATA_OUTPUT_FIELD_T_ALL == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/t_all/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/t_all/Value/N", m_nT_all);
				pUpdateItem->AddValue("/AttributeUpdates/t_all/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbDATA_OUTPUT_FIELD_T_INC == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/t_inc/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/t_inc/Value/N", m_nT_inc);
				pUpdateItem->AddValue("/AttributeUpdates/t_inc/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbDATA_OUTPUT_FIELD_DATA == iter->first)
		{
			if (!m_jData.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/data/Value/S", JsonEncode(oJsonWriter.write(m_jData), sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/data/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/data/Action", "DELETE");
			}
			continue;
		}
		if (TbDATA_OUTPUT_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbData_output::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbData_output::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbData_output::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/utype/N", m_nUtype);
		pItem->AddValue("/t_all/N", m_nT_all);
		pItem->AddValue("/t_inc/N", m_nT_inc);
		if (!m_jData.empty())
		{
				pItem->AddValue("/AttributeUpdates/data/Value/S", JsonEncode(oJsonWriter.write(m_jData), sJsonEncode));
		}
...
void TbData_output::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbData_output::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbData_output::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbData_output::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbData_output::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbData_output::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbData_output::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbData_output::OnGetItemReq(unsigned int udwIdxNo,
...
int TbData_output::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbData_output::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbData_output::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/utype/N", m_nUtype);
	pPutItem->AddValue("/Item/t_all/N", m_nT_all);
	pPutItem->AddValue("/Item/t_inc/N", m_nT_inc);
	if (!m_jData.empty())
	{
				pPutItem->AddValue("/Item/data/S", JsonEncode(oJsonWriter.write(m_jData), sJsonEncode));
	}
...
int TbData_output::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbData_output::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbData_output::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "utype")
		{
			m_nUtype = strtoll(item["utype"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "t_all")
		{
			m_nT_all = strtoll(item["t_all"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "t_inc")
		{
			m_nT_inc = strtoll(item["t_inc"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "data")
		{
			if (FALSE == oJsonReader.parse(item["data"]["S"].asString(),m_jData))
			{
				m_jData = Json::Value(Json::nullValue);
			}
			continue;
		}
		if (vecMembers[i] == "seq")
