...
TableDesc TbMystery_store::oTableDesc;

int TbMystery_store::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbMystery_store::NewObject()
{
	return new TbMystery_store;
}

string TbMystery_store::GetTableName()
{
	ostringstream oss;
...
TINT32 TbMystery_store::GetTableIdx()
{
	 return 0;
}

AwsMap* TbMystery_store::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbMystery_store::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbMystery_store::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbMystery_store::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbMYSTERY_STORE_FIELD_UID)
			{
...
int TbMystery_store::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbMystery_store::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbMystery_store::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbMystery_store::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbMYSTERY_STORE_FIELD_REFRESH_TIME == iter->first)
		{
		...
		if (TbMYSTERY_STORE_FIELD_PACKAGE == iter->first)
		{
			if (!m_bPackage.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/package/Value/B", Base64Encode((char*)&m_bPackage.m_astList[0], m_bPackage.m_udwNum*sizeof(SMysteryStorePackage), sBase64Encode));
				pUpdateItem->AddValue("/AttributeUpdates/package/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/package/Action", "DELETE");
			}
			continue;
		}
		if (TbMYSTERY_STORE_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbMystery_store::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbMystery_store::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbMystery_store::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		if (!m_bPackage.empty())
		{
			pItem->AddValue("/package/B", Base64Encode((char*)&m_bPackage.m_astList[0], m_bPackage.m_udwNum*sizeof(SMysteryStorePackage), sBase64Encode));
		}
...
void TbMystery_store::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbMystery_store::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbMystery_store::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbMystery_store::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbMystery_store::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbMystery_store::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbMystery_store::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbMystery_store::OnGetItemReq(unsigned int udwIdxNo,
...
int TbMystery_store::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbMystery_store::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbMystery_store::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	if (!m_bPackage.empty())
	{
		pPutItem->AddValue("/Item/package/B", Base64Encode((char*)&m_bPackage.m_astList[0], m_bPackage.m_udwNum*sizeof(SMysteryStorePackage), sBase64Encode));
	}
...
int TbMystery_store::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbMystery_store::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbMystery_store::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "package")
		{
			Base64Decode(item["package"]["B"].asString(), (char*)&m_bPackage.m_astList[0], dwResLen);
			m_bPackage.m_udwNum = dwResLen/sizeof(SMysteryStorePackage);
			if (0==m_bPackage.m_udwNum && dwResLen>0)
			{
				m_bPackage.m_udwNum = 1;
			}
			continue;
		}
		if (vecMembers[i] == "seq")
