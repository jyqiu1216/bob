#ifndef _AWS_TABLE_LORD_IMAGE_H_
#define _AWS_TABLE_LORD_IMAGE_H_

#include "aws_table_common.h"
#include "field_property.h"

#define EN_AWS_TABLE_LORD_IMAGE "lord_image"

enum ETbLord_imageField
{
	TbLORD_IMAGE_FIELD_UID = 0,
	TbLORD_IMAGE_FIELD_LORD_IMAGE = 1,
	TbLORD_IMAGE_FIELD_SEQ = 2,
};

enum ETbLord_imageOpenType
{
	ETbLORDIMAGE_OPEN_TYPE_PRIMARY = 0,
};

class TbLord_image : public AwsTable
{
public:
	TINT64 m_nUid;
	Json::Value m_jLord_image;
	TINT64 m_nSeq;
...
public:
	TbLord_image():AwsTable(&oTableDesc)
	{
		Reset();
	}
	~TbLord_image()
	...
	const Json::Value& Get_Lord_image()
	{
		return m_jLord_image;
	}
	void Set_Lord_image(const Json::Value& jLord_image)
	{
		m_jLord_image = jLord_image;
		m_mFlag[TbLORD_IMAGE_FIELD_LORD_IMAGE] = jLord_image.empty() ? UPDATE_ACTION_TYPE_DELETE : UPDATE_ACTION_TYPE_PUT;
	}
	...
	{
		m_nSeq = nSeq;
		m_mFlag[TbLORD_IMAGE_FIELD_SEQ] = dwActionType;
	}
	//Scan命令使用,返回完整的请求包
