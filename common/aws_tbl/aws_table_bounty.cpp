...
TableDesc TbBounty::oTableDesc;

int TbBounty::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbBounty::NewObject()
{
	return new TbBounty;
}

string TbBounty::GetTableName()
{
	ostringstream oss;
...
TINT32 TbBounty::GetTableIdx()
{
	 return 0;
}

AwsMap* TbBounty::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbBounty::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbBounty::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBounty::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbBOUNTY_FIELD_UID)
			{
...
int TbBounty::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbBounty::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbBounty::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbBounty::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbBOUNTY_FIELD_NEXT_REFRESH_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/next_refresh_time/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/next_refresh_time/Value/N", m_nNext_refresh_time);
				pUpdateItem->AddValue("/AttributeUpdates/next_refresh_time/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbBOUNTY_FIELD_STAR == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/star/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/star/Value/N", m_nStar);
				pUpdateItem->AddValue("/AttributeUpdates/star/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbBOUNTY_FIELD_S_STAGE == iter->first)
		{
			if (!m_jS_stage.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/s_stage/Value/S", JsonEncode(oJsonWriter.write(m_jS_stage), sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/s_stage/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/s_stage/Action", "DELETE");
			}
			continue;
		}
		if (TbBOUNTY_FIELD_S_REWARD == iter->first)
		{
			if (!m_jS_reward.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/s_reward/Value/S", JsonEncode(oJsonWriter.write(m_jS_reward), sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/s_reward/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/s_reward/Action", "DELETE");
			}
			continue;
		}
		if (TbBOUNTY_FIELD_BASE == iter->first)
		{
			if (!m_jBase.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/base/Value/S", JsonEncode(oJsonWriter.write(m_jBase), sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/base/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/base/Action", "DELETE");
			}
			continue;
		}
		if (TbBOUNTY_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbBounty::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbBounty::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBounty::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/next_refresh_time/N", m_nNext_refresh_time);
		pItem->AddValue("/star/N", m_nStar);
		if (!m_jS_stage.empty())
		{
				pItem->AddValue("/AttributeUpdates/s_stage/Value/S", JsonEncode(oJsonWriter.write(m_jS_stage), sJsonEncode));
		}
		if (!m_jS_reward.empty())
		{
				pItem->AddValue("/AttributeUpdates/s_reward/Value/S", JsonEncode(oJsonWriter.write(m_jS_reward), sJsonEncode));
		}
		if (!m_jBase.empty())
		{
				pItem->AddValue("/AttributeUpdates/base/Value/S", JsonEncode(oJsonWriter.write(m_jBase), sJsonEncode));
		}
...
void TbBounty::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbBounty::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbBounty::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbBounty::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBounty::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbBounty::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbBounty::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBounty::OnGetItemReq(unsigned int udwIdxNo,
...
int TbBounty::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbBounty::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBounty::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/next_refresh_time/N", m_nNext_refresh_time);
	pPutItem->AddValue("/Item/star/N", m_nStar);
	if (!m_jS_stage.empty())
	{
				pPutItem->AddValue("/Item/s_stage/S", JsonEncode(oJsonWriter.write(m_jS_stage), sJsonEncode));
	}
	if (!m_jS_reward.empty())
	{
				pPutItem->AddValue("/Item/s_reward/S", JsonEncode(oJsonWriter.write(m_jS_reward), sJsonEncode));
	}
	if (!m_jBase.empty())
	{
				pPutItem->AddValue("/Item/base/S", JsonEncode(oJsonWriter.write(m_jBase), sJsonEncode));
	}
...
int TbBounty::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbBounty::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbBounty::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "next_refresh_time")
		{
			m_nNext_refresh_time = strtoll(item["next_refresh_time"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "star")
		{
			m_nStar = strtoll(item["star"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "s_stage")
		{
			if (FALSE == oJsonReader.parse(item["s_stage"]["S"].asString(),m_jS_stage))
			{
				m_jS_stage = Json::Value(Json::nullValue);
			}
			continue;
		}
		if (vecMembers[i] == "s_reward")
		{
			if (FALSE == oJsonReader.parse(item["s_reward"]["S"].asString(),m_jS_reward))
			{
				m_jS_reward = Json::Value(Json::nullValue);
			}
			continue;
		}
		if (vecMembers[i] == "base")
		{
			if (FALSE == oJsonReader.parse(item["base"]["S"].asString(),m_jBase))
			{
				m_jBase = Json::Value(Json::nullValue);
			}
			continue;
		}
		if (vecMembers[i] == "seq")
