...
TableDesc TbDiplomacy::oTableDesc;

int TbDiplomacy::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbDiplomacy::NewObject()
{
	return new TbDiplomacy;
}

string TbDiplomacy::GetTableName()
{
	ostringstream oss;
...
TINT32 TbDiplomacy::GetTableIdx()
{
	 return 0;
}

AwsMap* TbDiplomacy::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
	 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
	...
	if (bHasStartKey)
	{
		pScan->AddValue("/ExclusiveStartKey/src_al/N",m_nSrc_al);
		pScan->AddValue("/ExclusiveStartKey/des_al/N",m_nDes_al);
	}
	return pScan;
}

int TbDiplomacy::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbDiplomacy::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbDiplomacy::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbDIPLOMACY_FIELD_SRC_AL)
			{
				pQuery->AddValue("/KeyConditions/src_al/AttributeValueList[0]/N", m_nSrc_al);
				pQuery->AddValue("/KeyConditions/src_al/ComparisonOperator", "EQ");
			}
		}
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbDIPLOMACY_FIELD_DES_AL)
				{
					pQuery->AddValue("/KeyConditions/des_al/AttributeValueList[0]/N", m_nDes_al);
					pQuery->AddValue("/KeyConditions/des_al/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				if(fld_desc.udwFldNo == TbDIPLOMACY_FIELD_DES_AL)
				{
					pQuery->AddValue("/KeyConditions/des_al/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/des_al/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/des_al/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbDiplomacy::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbDiplomacy::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbDiplomacy::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbDiplomacy::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	pUpdateItem->AddValue("/Key/src_al/N", m_nSrc_al);
	pUpdateItem->AddValue("/Key/des_al/N", m_nDes_al);
	for (map<unsigned int, int>::iterator iter = m_mFlag.begin(); iter != m_mFlag.end(); ++iter)
	{
		bUpdateFlag = true;
		if (TbDIPLOMACY_FIELD_TYPE == iter->first)
		{
		...
		if (TbDIPLOMACY_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbDiplomacy::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbDiplomacy::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbDiplomacy::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/src_al/N", m_nSrc_al);
		pItem->AddValue("/des_al/N", m_nDes_al);
...
void TbDiplomacy::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbDiplomacy::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
	...
	assert(pReadItem);
	pReadItem->AddValue("/Keys[0]/src_al/N", m_nSrc_al);
	pReadItem->AddValue("/Keys[0]/des_al/N", m_nDes_al);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
void TbDiplomacy::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
	AwsList* pKeys = pReadItem->GetAwsMap("RequestItems")->GetAwsMap(GetTableName())->GetAwsList("Keys");
	AwsMap* pKey = new AwsMap;
	assert(pKey);
	pKey->AddValue("/src_al/N", m_nSrc_al);
	pKey->AddValue("/des_al/N", m_nDes_al);
	pKeys->SetValue(pKey, true);
}

int TbDiplomacy::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbDiplomacy::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pDeleteItem->AddValue("/Key/src_al/N", m_nSrc_al);
	pDeleteItem->AddValue("/Key/des_al/N", m_nDes_al);
	ostringstream oss;
	for (unsigned int i = 0; i < expected_desc.vecExpectedItem.size(); ++i)
	{
		const ExpectedItem& item = expected_desc.vecExpectedItem[i];
...
int TbDiplomacy::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbDiplomacy::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbDiplomacy::OnGetItemReq(unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
	...
	pGetItem->AddValue("/Key/src_al/N", m_nSrc_al);
	pGetItem->AddValue("/Key/des_al/N", m_nDes_al);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
int TbDiplomacy::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbDiplomacy::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbDiplomacy::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/src_al/N", m_nSrc_al);
	pPutItem->AddValue("/Item/des_al/N", m_nDes_al);
...
int TbDiplomacy::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbDiplomacy::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbDiplomacy::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	{
 		if (vecMembers[i] == "src_al")
		{
			m_nSrc_al = strtoll(item["src_al"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "des_al")
		{
			m_nDes_al = strtoll(item["des_al"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
