...
TableDesc TbReport_user::oTableDesc;

int TbReport_user::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbReport_user::NewObject()
{
	return new TbReport_user;
}

string TbReport_user::GetTableName()
{
	ostringstream oss;
...
TINT32 TbReport_user::GetTableIdx()
{
	 return 0;
}

AwsMap* TbReport_user::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
	 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
	...
	if (bHasStartKey)
	{
		...
		pScan->AddValue("/ExclusiveStartKey/rid/N",m_nRid);
	}
	return pScan;
}

int TbReport_user::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbReport_user::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbReport_user::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbREPORT_USER_FIELD_UID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbREPORT_USER_FIELD_RID)
				{
					pQuery->AddValue("/KeyConditions/rid/AttributeValueList[0]/N", m_nRid);
					pQuery->AddValue("/KeyConditions/rid/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				if(fld_desc.udwFldNo == TbREPORT_USER_FIELD_RID)
				{
					pQuery->AddValue("/KeyConditions/rid/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/rid/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/rid/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbReport_user::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbReport_user::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbReport_user::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbReport_user::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	pUpdateItem->AddValue("/Key/rid/N", m_nRid);
	for (map<unsigned int, int>::iterator iter = m_mFlag.begin(); iter != m_mFlag.end(); ++iter)
	{
		bUpdateFlag = true;
		if (TbREPORT_USER_FIELD_TYPE == iter->first)
		{
		...
		if (TbREPORT_USER_FIELD_REPORT_TYPE == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/report_type/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/report_type/Value/N", m_nReport_type);
				pUpdateItem->AddValue("/AttributeUpdates/report_type/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbREPORT_USER_FIELD_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbREPORT_USER_FIELD_STATUS == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbREPORT_USER_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbReport_user::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbReport_user::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbReport_user::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/rid/N", m_nRid);
		pItem->AddValue("/type/N", m_nType);
		pItem->AddValue("/report_type/N", m_nReport_type);
...
void TbReport_user::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbReport_user::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
	...
	pReadItem->AddValue("/Keys[0]/rid/N", m_nRid);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
void TbReport_user::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
	...
	pKey->AddValue("/rid/N", m_nRid);
	pKeys->SetValue(pKey, true);
}

int TbReport_user::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbReport_user::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pDeleteItem->AddValue("/Key/rid/N", m_nRid);
	ostringstream oss;
	for (unsigned int i = 0; i < expected_desc.vecExpectedItem.size(); ++i)
	{
		const ExpectedItem& item = expected_desc.vecExpectedItem[i];
...
int TbReport_user::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbReport_user::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbReport_user::OnGetItemReq(unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
	...
	pGetItem->AddValue("/Key/rid/N", m_nRid);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
int TbReport_user::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbReport_user::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbReport_user::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/rid/N", m_nRid);
	pPutItem->AddValue("/Item/type/N", m_nType);
	pPutItem->AddValue("/Item/report_type/N", m_nReport_type);
...
int TbReport_user::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbReport_user::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbReport_user::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "rid")
		{
			m_nRid = strtoll(item["rid"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		...
		if (vecMembers[i] == "report_type")
		{
			m_nReport_type = strtoll(item["report_type"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
