...
TableDesc TbUnique_name::oTableDesc;

int TbUnique_name::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbUnique_name::NewObject()
{
	return new TbUnique_name;
}

string TbUnique_name::GetTableName()
{
	ostringstream oss;
...
TINT32 TbUnique_name::GetTableIdx()
{
	 return 0;
}

AwsMap* TbUnique_name::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
	 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
	...
	if (bHasStartKey)
	{
		pScan->AddValue("/ExclusiveStartKey/name/S",m_sName);
		pScan->AddValue("/ExclusiveStartKey/type/N",m_nType);
	}
	return pScan;
}

int TbUnique_name::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbUnique_name::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbUnique_name::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbUNIQUE_NAME_FIELD_NAME)
			{
				pQuery->AddValue("/KeyConditions/name/AttributeValueList[0]/S", m_sName);
				pQuery->AddValue("/KeyConditions/name/ComparisonOperator", "EQ");
			}
		}
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbUNIQUE_NAME_FIELD_TYPE)
				{
					pQuery->AddValue("/KeyConditions/type/AttributeValueList[0]/N", m_nType);
					pQuery->AddValue("/KeyConditions/type/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				if(fld_desc.udwFldNo == TbUNIQUE_NAME_FIELD_TYPE)
				{
					pQuery->AddValue("/KeyConditions/type/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/type/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/type/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbUnique_name::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbUnique_name::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbUnique_name::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbUnique_name::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	pUpdateItem->AddValue("/Key/name/S", m_sName);
	pUpdateItem->AddValue("/Key/type/N", m_nType);
	for (map<unsigned int, int>::iterator iter = m_mFlag.begin(); iter != m_mFlag.end(); ++iter)
	{
		bUpdateFlag = true;
		if (TbUNIQUE_NAME_FIELD_EXIST == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/exist/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/exist/Value/N", m_nExist);
				pUpdateItem->AddValue("/AttributeUpdates/exist/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbUNIQUE_NAME_FIELD_ID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/id/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/id/Value/N", m_nId);
				pUpdateItem->AddValue("/AttributeUpdates/id/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbUNIQUE_NAME_FIELD_PROFILE == iter->first)
		{
			if (!m_jProfile.empty())
			{
		...
		if (TbUNIQUE_NAME_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbUnique_name::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbUnique_name::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbUnique_name::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/exist/N", m_nExist);
...
void TbUnique_name::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbUnique_name::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
	...
	assert(pReadItem);
	pReadItem->AddValue("/Keys[0]/name/S", m_sName);
	pReadItem->AddValue("/Keys[0]/type/N", m_nType);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
void TbUnique_name::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
	AwsList* pKeys = pReadItem->GetAwsMap("RequestItems")->GetAwsMap(GetTableName())->GetAwsList("Keys");
	AwsMap* pKey = new AwsMap;
	assert(pKey);
	pKey->AddValue("/name/S", m_sName);
	pKey->AddValue("/type/N", m_nType);
	pKeys->SetValue(pKey, true);
}

int TbUnique_name::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbUnique_name::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pDeleteItem->AddValue("/Key/name/S", m_sName);
	pDeleteItem->AddValue("/Key/type/N", m_nType);
	ostringstream oss;
	for (unsigned int i = 0; i < expected_desc.vecExpectedItem.size(); ++i)
	{
		const ExpectedItem& item = expected_desc.vecExpectedItem[i];
...
int TbUnique_name::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbUnique_name::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbUnique_name::OnGetItemReq(unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
	...
	pGetItem->AddValue("/Key/name/S", m_sName);
	pGetItem->AddValue("/Key/type/N", m_nType);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
int TbUnique_name::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbUnique_name::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbUnique_name::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/exist/N", m_nExist);
...
int TbUnique_name::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbUnique_name::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbUnique_name::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "exist")
		{
			m_nExist = strtoll(item["exist"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "id")
		{
