...
TableDesc TbRally_history::oTableDesc;

int TbRally_history::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbRally_history::NewObject()
{
	return new TbRally_history;
}

string TbRally_history::GetTableName()
{
	ostringstream oss;
...
TINT32 TbRally_history::GetTableIdx()
{
	 return 0;
}

AwsMap* TbRally_history::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbRally_history::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbRally_history::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRally_history::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbRALLY_HISTORY_FIELD_AID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbRALLY_HISTORY_FIELD_RID)
				{
...
int TbRally_history::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbRally_history::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbRally_history::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbRally_history::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbRALLY_HISTORY_FIELD_CONTENT == iter->first)
		{
			if (!m_jContent.empty())
			{
		...
		if (TbRALLY_HISTORY_FIELD_SEQ == iter->first)
		{
...
int TbRally_history::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbRally_history::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRally_history::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
...
void TbRally_history::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbRally_history::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbRally_history::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbRally_history::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRally_history::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbRally_history::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbRally_history::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRally_history::OnGetItemReq(unsigned int udwIdxNo,
...
int TbRally_history::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbRally_history::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRally_history::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbRally_history::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbRally_history::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbRally_history::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
