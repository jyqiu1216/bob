...
TableDesc TbAction::oTableDesc;

int TbAction::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbAction::NewObject()
{
	return new TbAction;
}

string TbAction::GetTableName()
{
	ostringstream oss;
...
TINT32 TbAction::GetTableIdx()
{
	 return 0;
}

AwsMap* TbAction::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbAction::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbAction::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbAction::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbACTION_FIELD_SUID)
			{
			...
			if(fld_desc.udwFldNo == TbACTION_FIELD_SID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbACTION_FIELD_ID)
				{
				...
				if(fld_desc.udwFldNo == TbACTION_FIELD_ETIME)
				{
...
int TbAction::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbAction::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbAction::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbAction::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbACTION_FIELD_MCLASS == iter->first)
		{
		...
		if (TbACTION_FIELD_SCLASS == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_STATUS == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_BTIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_CTIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_ETIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_PARAM == iter->first)
		{
			if (!m_bParam.empty())
			{
		...
		if (TbACTION_FIELD_RETRY == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_SID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_NOTI_FLAG == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_SCID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbACTION_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbAction::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbAction::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbAction::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
...
void TbAction::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbAction::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbAction::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbAction::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbAction::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbAction::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbAction::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbAction::OnGetItemReq(unsigned int udwIdxNo,
...
int TbAction::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbAction::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbAction::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbAction::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbAction::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbAction::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
