...
TableDesc TbSvr_al::oTableDesc;

int TbSvr_al::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbSvr_al::NewObject()
{
	return new TbSvr_al;
}

string TbSvr_al::GetTableName()
{
	ostringstream oss;
...
TINT32 TbSvr_al::GetTableIdx()
{
	 return 0;
}

AwsMap* TbSvr_al::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbSvr_al::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbSvr_al::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbSvr_al::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbSVR_AL_FIELD_SID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbSVR_AL_FIELD_ALID)
				{
					pQuery->AddValue("/KeyConditions/alid/AttributeValueList[0]/N", m_nAlid);
					pQuery->AddValue("/KeyConditions/alid/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				if(fld_desc.udwFldNo == TbSVR_AL_FIELD_ALID)
				{
					pQuery->AddValue("/KeyConditions/alid/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/alid/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/alid/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbSvr_al::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbSvr_al::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbSvr_al::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbSvr_al::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbSVR_AL_FIELD_OWNER_UID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/owner_uid/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/owner_uid/Value/N", m_nOwner_uid);
				pUpdateItem->AddValue("/AttributeUpdates/owner_uid/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbSVR_AL_FIELD_OWNER_CID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbSVR_AL_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbSvr_al::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbSvr_al::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbSvr_al::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/owner_uid/N", m_nOwner_uid);
...
void TbSvr_al::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbSvr_al::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbSvr_al::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbSvr_al::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbSvr_al::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbSvr_al::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbSvr_al::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbSvr_al::OnGetItemReq(unsigned int udwIdxNo,
...
int TbSvr_al::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbSvr_al::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbSvr_al::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/owner_uid/N", m_nOwner_uid);
...
int TbSvr_al::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbSvr_al::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbSvr_al::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "owner_uid")
		{
			m_nOwner_uid = strtoll(item["owner_uid"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
