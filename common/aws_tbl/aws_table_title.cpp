...
TableDesc TbTitle::oTableDesc;

int TbTitle::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbTitle::NewObject()
{
	return new TbTitle;
}

string TbTitle::GetTableName()
{
	ostringstream oss;
...
TINT32 TbTitle::GetTableIdx()
{
	 return 0;
}

AwsMap* TbTitle::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbTitle::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbTitle::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTitle::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbTITLE_FIELD_SID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbTITLE_FIELD_ID)
				{
...
int TbTitle::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbTitle::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbTitle::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbTitle::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbTITLE_FIELD_UID == iter->first)
		{
		...
		if (TbTITLE_FIELD_NAME == iter->first)
		{
			if (!m_sName.empty())
			{
		...
		if (TbTITLE_FIELD_ALNICK == iter->first)
		{
			if (!m_sAlnick.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/alnick/Value/S", JsonEncode(m_sAlnick, sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/alnick/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/alnick/Action", "DELETE");
			}
			continue;
		}
		if (TbTITLE_FIELD_ALID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbTITLE_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbTITLE_FIELD_DUB_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/dub_time/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/dub_time/Value/N", m_nDub_time);
				pUpdateItem->AddValue("/AttributeUpdates/dub_time/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbTITLE_FIELD_CID == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbTitle::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbTitle::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTitle::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		if (!m_sAlnick.empty())
		{
			pItem->AddValue("/alnick/S", JsonEncode(m_sAlnick, sJsonEncode));
		}
		pItem->AddValue("/alid/N", m_nAlid);
		pItem->AddValue("/seq/N", m_nSeq);
		pItem->AddValue("/dub_time/N", m_nDub_time);
...
void TbTitle::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbTitle::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbTitle::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbTitle::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTitle::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbTitle::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbTitle::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTitle::OnGetItemReq(unsigned int udwIdxNo,
...
int TbTitle::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbTitle::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTitle::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	if (!m_sAlnick.empty())
	{
		pPutItem->AddValue("/Item/alnick/S", JsonEncode(m_sAlnick, sJsonEncode));
	}
	pPutItem->AddValue("/Item/alid/N", m_nAlid);
	pPutItem->AddValue("/Item/seq/N", m_nSeq);
	pPutItem->AddValue("/Item/dub_time/N", m_nDub_time);
...
int TbTitle::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbTitle::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbTitle::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "alnick")
		{
			m_sAlnick = item["alnick"]["S"].asString();
			continue;
		}
		...
		if (vecMembers[i] == "dub_time")
		{
			m_nDub_time = strtoll(item["dub_time"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "cid")
		{
