...
TableDesc TbEvent_tips::oTableDesc;

int TbEvent_tips::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbEvent_tips::NewObject()
{
	return new TbEvent_tips;
}

string TbEvent_tips::GetTableName()
{
	ostringstream oss;
...
TINT32 TbEvent_tips::GetTableIdx()
{
	 return 0;
}

AwsMap* TbEvent_tips::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbEvent_tips::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbEvent_tips::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbEvent_tips::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbEVENT_TIPS_FIELD_UID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbEVENT_TIPS_FIELD_ID)
				{
				...
				if(fld_desc.udwFldNo == TbEVENT_TIPS_FIELD_TIME)
				{
					pQuery->AddValue("/KeyConditions/time/AttributeValueList[0]/N", m_nTime);
					pQuery->AddValue("/KeyConditions/time/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				...
				if(fld_desc.udwFldNo == TbEVENT_TIPS_FIELD_TIME)
				{
					pQuery->AddValue("/KeyConditions/time/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/time/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/time/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbEvent_tips::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbEvent_tips::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbEvent_tips::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbEvent_tips::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbEVENT_TIPS_FIELD_CONTENT == iter->first)
		{
			if (!m_jContent.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/content/Value/S", JsonEncode(oJsonWriter.write(m_jContent), sJsonEncode));
		...
		if (TbEVENT_TIPS_FIELD_TIME == iter->first)
		{
		...
		if (TbEVENT_TIPS_FIELD_EVENT_END_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/event_end_time/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/event_end_time/Value/N", m_nEvent_end_time);
				pUpdateItem->AddValue("/AttributeUpdates/event_end_time/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbEVENT_TIPS_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbEvent_tips::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbEvent_tips::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbEvent_tips::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		if (!m_jContent.empty())
		{
				pItem->AddValue("/AttributeUpdates/content/Value/S", JsonEncode(oJsonWriter.write(m_jContent), sJsonEncode));
		}
		pItem->AddValue("/time/N", m_nTime);
		pItem->AddValue("/event_end_time/N", m_nEvent_end_time);
...
void TbEvent_tips::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbEvent_tips::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbEvent_tips::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbEvent_tips::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbEvent_tips::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbEvent_tips::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbEvent_tips::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbEvent_tips::OnGetItemReq(unsigned int udwIdxNo,
...
int TbEvent_tips::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbEvent_tips::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbEvent_tips::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	if (!m_jContent.empty())
	{
				pPutItem->AddValue("/Item/content/S", JsonEncode(oJsonWriter.write(m_jContent), sJsonEncode));
	}
	pPutItem->AddValue("/Item/time/N", m_nTime);
	pPutItem->AddValue("/Item/event_end_time/N", m_nEvent_end_time);
...
int TbEvent_tips::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbEvent_tips::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbEvent_tips::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		{
			if (FALSE == oJsonReader.parse(item["content"]["S"].asString(),m_jContent))
			{
				m_jContent = Json::Value(Json::nullValue);
			}
			continue;
		}
		...
		if (vecMembers[i] == "event_end_time")
		{
			m_nEvent_end_time = strtoll(item["event_end_time"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "seq")
		{
