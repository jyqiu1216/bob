...
TableDesc TbRandom_reward::oTableDesc;

int TbRandom_reward::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbRandom_reward::NewObject()
{
	return new TbRandom_reward;
}

string TbRandom_reward::GetTableName()
{
	ostringstream oss;
...
TINT32 TbRandom_reward::GetTableIdx()
{
	 return 0;
}

AwsMap* TbRandom_reward::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbRandom_reward::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbRandom_reward::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRandom_reward::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbRANDOM_REWARD_FIELD_UID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbRANDOM_REWARD_FIELD_TYPE)
				{
...
int TbRandom_reward::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbRandom_reward::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbRandom_reward::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbRandom_reward::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbRANDOM_REWARD_FIELD_NUM == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/num/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/num/Value/N", m_nNum);
				pUpdateItem->AddValue("/AttributeUpdates/num/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbRANDOM_REWARD_FIELD_ETIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbRANDOM_REWARD_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbRandom_reward::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbRandom_reward::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRandom_reward::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
...
void TbRandom_reward::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbRandom_reward::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbRandom_reward::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbRandom_reward::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRandom_reward::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbRandom_reward::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbRandom_reward::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRandom_reward::OnGetItemReq(unsigned int udwIdxNo,
...
int TbRandom_reward::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbRandom_reward::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbRandom_reward::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/num/N", m_nNum);
...
int TbRandom_reward::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbRandom_reward::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbRandom_reward::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "num")
		{
			m_nNum = strtoll(item["num"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
