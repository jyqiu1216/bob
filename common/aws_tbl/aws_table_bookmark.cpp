...
TableDesc TbBookmark::oTableDesc;

int TbBookmark::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbBookmark::NewObject()
{
	return new TbBookmark;
}

string TbBookmark::GetTableName()
{
	ostringstream oss;
...
TINT32 TbBookmark::GetTableIdx()
{
	 return 0;
}

AwsMap* TbBookmark::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
	 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
	...
	if (bHasStartKey)
	{
		...
		pScan->AddValue("/ExclusiveStartKey/pos/N",m_nPos);
	}
	return pScan;
}

int TbBookmark::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbBookmark::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBookmark::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbBOOKMARK_FIELD_UID)
			{
		...
		else if (i == 1) //1只能是rangekey or local index，可以有多种方式
		{
			...
			{
				if(fld_desc.udwFldNo == TbBOOKMARK_FIELD_POS)
				{
					pQuery->AddValue("/KeyConditions/pos/AttributeValueList[0]/N", m_nPos);
					pQuery->AddValue("/KeyConditions/pos/ComparisonOperator", CompareType2Str(comp_desc.dwCompareType));
				}
			}
			else if (COMPARE_TYPE_BETWEEN == comp_desc.dwCompareType)
			{
				if(fld_desc.udwFldNo == TbBOOKMARK_FIELD_POS)
				{
					pQuery->AddValue("/KeyConditions/pos/AttributeValueList[0]/N" , comp_desc.vecN[0]);
					pQuery->AddValue("/KeyConditions/pos/AttributeValueList[1]/N" , comp_desc.vecN[1]);
					pQuery->AddValue("/KeyConditions/pos/ComparisonOperator", "BETWEEN");
				}
			}
			else
			{
...
int TbBookmark::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbBookmark::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbBookmark::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbBookmark::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	pUpdateItem->AddValue("/Key/pos/N", m_nPos);
	for (map<unsigned int, int>::iterator iter = m_mFlag.begin(); iter != m_mFlag.end(); ++iter)
	{
		bUpdateFlag = true;
		if (TbBOOKMARK_FIELD_FLAG == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/flag/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/flag/Value/N", m_nFlag);
				pUpdateItem->AddValue("/AttributeUpdates/flag/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbBOOKMARK_FIELD_NICK == iter->first)
		{
			if (!m_sNick.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/nick/Value/S", JsonEncode(m_sNick, sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/nick/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/nick/Action", "DELETE");
			}
			continue;
		}
		if (TbBOOKMARK_FIELD_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
		...
		if (TbBOOKMARK_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbBookmark::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbBookmark::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBookmark::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/flag/N", m_nFlag);
		if (!m_sNick.empty())
		{
			pItem->AddValue("/nick/S", JsonEncode(m_sNick, sJsonEncode));
		}
...
void TbBookmark::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbBookmark::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
	...
	pReadItem->AddValue("/Keys[0]/pos/N", m_nPos);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
void TbBookmark::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
	...
	pKey->AddValue("/pos/N", m_nPos);
	pKeys->SetValue(pKey, true);
}

int TbBookmark::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBookmark::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pDeleteItem->AddValue("/Key/pos/N", m_nPos);
	ostringstream oss;
	for (unsigned int i = 0; i < expected_desc.vecExpectedItem.size(); ++i)
	{
		const ExpectedItem& item = expected_desc.vecExpectedItem[i];
...
int TbBookmark::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbBookmark::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBookmark::OnGetItemReq(unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
	...
	pGetItem->AddValue("/Key/pos/N", m_nPos);
	ostringstream oss;
	for (unsigned int i = 0; i < idx_desc.vecRtnFld.size(); ++i)
	{
		oss.str("");
...
int TbBookmark::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbBookmark::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbBookmark::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/flag/N", m_nFlag);
	if (!m_sNick.empty())
	{
		pPutItem->AddValue("/Item/nick/S", JsonEncode(m_sNick, sJsonEncode));
	}
...
int TbBookmark::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbBookmark::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbBookmark::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "flag")
		{
			m_nFlag = strtoll(item["flag"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "nick")
		{
			m_sNick = item["nick"]["S"].asString();
			continue;
		}
