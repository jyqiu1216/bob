...
TableDesc TbLord_image::oTableDesc;

int TbLord_image::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbLord_image::NewObject()
{
	return new TbLord_image;
}

string TbLord_image::GetTableName()
{
	ostringstream oss;
...
TINT32 TbLord_image::GetTableIdx()
{
	 return 0;
}

AwsMap* TbLord_image::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbLord_image::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbLord_image::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbLord_image::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbLORD_IMAGE_FIELD_UID)
			{
...
int TbLord_image::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbLord_image::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbLord_image::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbLord_image::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbLORD_IMAGE_FIELD_LORD_IMAGE == iter->first)
		{
			if (!m_jLord_image.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/lord_image/Value/S", JsonEncode(oJsonWriter.write(m_jLord_image), sJsonEncode));
				pUpdateItem->AddValue("/AttributeUpdates/lord_image/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/lord_image/Action", "DELETE");
			}
			continue;
		}
		if (TbLORD_IMAGE_FIELD_SEQ == iter->first)
		{
...
int TbLord_image::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbLord_image::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbLord_image::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		if (!m_jLord_image.empty())
		{
				pItem->AddValue("/AttributeUpdates/lord_image/Value/S", JsonEncode(oJsonWriter.write(m_jLord_image), sJsonEncode));
		}
...
void TbLord_image::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbLord_image::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbLord_image::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbLord_image::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbLord_image::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbLord_image::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbLord_image::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbLord_image::OnGetItemReq(unsigned int udwIdxNo,
...
int TbLord_image::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbLord_image::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbLord_image::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	if (!m_jLord_image.empty())
	{
				pPutItem->AddValue("/Item/lord_image/S", JsonEncode(oJsonWriter.write(m_jLord_image), sJsonEncode));
	}
...
int TbLord_image::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbLord_image::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbLord_image::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "lord_image")
		{
			if (FALSE == oJsonReader.parse(item["lord_image"]["S"].asString(),m_jLord_image))
			{
				m_jLord_image = Json::Value(Json::nullValue);
			}
			continue;
		}
		if (vecMembers[i] == "seq")
