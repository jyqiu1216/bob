#ifndef _AWS_TABLE_BLACK_ACCOUNT_H_
#define _AWS_TABLE_BLACK_ACCOUNT_H_

#include "aws_table_common.h"
#include "field_property.h"

#define EN_AWS_TABLE_BLACK_ACCOUNT "black_account"

enum ETbBlack_accountField
{
	TbBLACK_ACCOUNT_FIELD_KEY = 0,
	TbBLACK_ACCOUNT_FIELD_DESC = 1,
	TbBLACK_ACCOUNT_FIELD_SEQ = 2,
};

enum ETbBlack_accountOpenType
{
	ETbBLACKACCOUNT_OPEN_TYPE_PRIMARY = 0,
};

class TbBlack_account : public AwsTable
{
public:
	string m_sKey;
	string m_sDesc;
...
public:
	TbBlack_account():AwsTable(&oTableDesc)
	{
		Reset();
	}
	~TbBlack_account()
	...
	void Set_Key(const string& sKey)
	{
		m_sKey = sKey;
	}
	...
	{
		m_sDesc = sDesc;
		m_mFlag[TbBLACK_ACCOUNT_FIELD_DESC] = sDesc.empty() ? UPDATE_ACTION_TYPE_DELETE : UPDATE_ACTION_TYPE_PUT;
	}
	...
	{
		m_nSeq = nSeq;
		m_mFlag[TbBLACK_ACCOUNT_FIELD_SEQ] = dwActionType;
	}
	//Scan命令使用,返回完整的请求包
