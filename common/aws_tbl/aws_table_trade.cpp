...
TableDesc TbTrade::oTableDesc;

int TbTrade::Init(const string& sConfFile, const string strProjectName)
{
...
AwsTable* TbTrade::NewObject()
{
	return new TbTrade;
}

string TbTrade::GetTableName()
{
	ostringstream oss;
...
TINT32 TbTrade::GetTableIdx()
{
	 return 0;
}

AwsMap* TbTrade::OnScanReq(unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
...
int TbTrade::OnScanReq(string& sPostData, unsigned int udwIdxNo, bool bHasStartKey, bool bReturnConsumedCapacity,
 unsigned int dwLimit, unsigned int dwSegment, unsigned int dwTotalSegments)
{
...
int TbTrade::OnScanRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTrade::OnQueryReq(unsigned int udwIdxNo, const CompareDesc& comp_desc,
	bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit, bool bCount)
{
	...
	for (unsigned int i = 0; i < idx_desc.vecIdxFld.size(); ++i)
	{
		...
		{
			if(fld_desc.udwFldNo == TbTRADE_FIELD_UID)
			{
...
int TbTrade::OnQueryReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc,
...
int TbTrade::OnQueryRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbTrade::OnCountReq(string& sPostData, unsigned int udwIdxNo, const CompareDesc& comp_desc, 
		bool bConsistentRead, bool bReturnConsumedCapacity, bool bScanIndexForward, unsigned int dwLimit)
{
...
AwsMap* TbTrade::OnUpdateItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	...
	{
		bUpdateFlag = true;
		if (TbTRADE_FIELD_SID == iter->first)
		{
		...
		if (TbTRADE_FIELD_REFRESH_TIME == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
				pUpdateItem->AddValue("/AttributeUpdates/refresh_time/Action", "DELETE");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/refresh_time/Value/N", m_nRefresh_time);
				pUpdateItem->AddValue("/AttributeUpdates/refresh_time/Action", UpdateActionType2Str(iter->second));
			}
			continue;
		}
		if (TbTRADE_FIELD_TRADE_LIST == iter->first)
		{
			if (!m_bTrade_list.empty())
			{
				pUpdateItem->AddValue("/AttributeUpdates/trade_list/Value/B", Base64Encode((char*)&m_bTrade_list.m_astList[0], m_bTrade_list.m_udwNum*sizeof(STradeList), sBase64Encode));
				pUpdateItem->AddValue("/AttributeUpdates/trade_list/Action", "PUT");
			}
			else
			{
				pUpdateItem->AddValue("/AttributeUpdates/trade_list/Action", "DELETE");
			}
			continue;
		}
		if (TbTRADE_FIELD_SEQ == iter->first)
		{
			if(UPDATE_ACTION_TYPE_DELETE == iter->second)
			{
...
int TbTrade::OnUpdateItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
...
int TbTrade::OnUpdateItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTrade::OnWriteItemReq(int dwActionType)
{
	++m_nSeq;
	oJsonWriter.omitEndingLineFeed();
	...
	if (WRITE_ACTION_TYPE_PUT == dwActionType)
	{
		...
		pItem->AddValue("/refresh_time/N", m_nRefresh_time);
		if (!m_bTrade_list.empty())
		{
			pItem->AddValue("/trade_list/B", Base64Encode((char*)&m_bTrade_list.m_astList[0], m_bTrade_list.m_udwNum*sizeof(STradeList), sBase64Encode));
		}
...
void TbTrade::OnWriteItemReq(AwsMap* pWriteItem, int dwActionType)
{
	++m_nSeq;
	assert(pWriteItem);
	string sBase64Encode;
...
AwsMap* TbTrade::OnReadItemReq(unsigned int udwIdxNo)
{
	oJsonWriter.omitEndingLineFeed();
	IndexDesc& idx_desc = oTableDesc.mIndexDesc[udwIdxNo];
	assert(idx_desc.sName == "PRIMARY"); //只能通过主键查询
...
void TbTrade::OnReadItemReq(AwsMap* pReadItem)
{
	assert(pReadItem);
...
int TbTrade::OnReadItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTrade::OnDeleteItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
...
int TbTrade::OnDeleteItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbTrade::OnDeleteItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTrade::OnGetItemReq(unsigned int udwIdxNo,
...
int TbTrade::OnGetItemReq(string& sPostData, unsigned int udwIdxNo,
	bool bConsistentRead, bool bReturnConsumedCapacity)
{
...
int TbTrade::OnGetItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

AwsMap* TbTrade::OnPutItemReq(
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	oJsonWriter.omitEndingLineFeed();
	...
	pPutItem->AddValue("/Item/refresh_time/N", m_nRefresh_time);
	if (!m_bTrade_list.empty())
	{
		pPutItem->AddValue("/Item/trade_list/B", Base64Encode((char*)&m_bTrade_list.m_astList[0], m_bTrade_list.m_udwNum*sizeof(STradeList), sBase64Encode));
	}
...
int TbTrade::OnPutItemReq(string& sPostData,
	const ExpectedDesc& expected_desc, int dwReturnValuesType, bool bReturnConsumedCapacity)
{
	++m_nSeq;
...
int TbTrade::OnPutItemRsp(const Json::Value& item)
{
	return OnResponse(item);
}

int TbTrade::OnResponse(const Json::Value& item)
{
	oJsonWriter.omitEndingLineFeed();
	int dwResLen = 0;
	dwResLen = 0;
	...
	for (unsigned int i = 0; i < vecMembers.size(); ++i)
	{
		...
		if (vecMembers[i] == "refresh_time")
		{
			m_nRefresh_time = strtoll(item["refresh_time"]["N"].asString().c_str(), NULL, 10);
			continue;
		}
		if (vecMembers[i] == "trade_list")
		{
			Base64Decode(item["trade_list"]["B"].asString(), (char*)&m_bTrade_list.m_astList[0], dwResLen);
			m_bTrade_list.m_udwNum = dwResLen/sizeof(STradeList);
			if (0==m_bTrade_list.m_udwNum && dwResLen>0)
			{
				m_bTrade_list.m_udwNum = 1;
			}
			continue;
		}
		if (vecMembers[i] == "seq")
